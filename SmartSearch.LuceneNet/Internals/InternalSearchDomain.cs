...
namespace SmartSearch.LuceneNet.Internals
{
    ...
    {
        public InternalSearchDomain(
            string name,
            IEnumerable<IField> fields,
            IEnumerable<ISpecializedField> specialiedFields,
            IAnalysisSettings analysisSettings
        )
        {
            Name = name;
            Fields = (fields ?? Array.Empty<IField>()).ToList().AsReadOnly();
            SpecializedFields = (specialiedFields ?? Array.Empty<ISpecializedField>())
                .ToList()
                .AsReadOnly();
        ...
        public IField[] GetFacetEnabledFields() => Fields.Where(f => f.EnableFaceting).ToArray();
        ...
        public static InternalSearchDomain CreateFrom(ISearchDomain domain)
        {
            ...
            return new InternalSearchDomain(
                domain.Name,
                domain.Fields,
                specializedFields,
                domain.AnalysisSettings
            );
        }

        private static List<ISpecializedField> BuildSpecializedFields(
            IEnumerable<IField> fields,
            IAnalysisSettings analysisSettings
        )
        {
